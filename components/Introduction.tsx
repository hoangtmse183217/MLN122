...
const interactiveContent = [
  {
    ...
    explanation:
      "Giống như một cái chợ sôi động, nền kinh tế nước ta chấp nhận cạnh tranh (các công ty đua nhau làm tốt hơn), giá cả thay đổi theo cung - cầu (hàng hiếm thì đắt, hàng thừa thì rẻ).",
    example:
      "Giá vé máy bay dịp Tết tăng cao do nhiều người muốn về quê (cầu tăng). Các hãng điện thoại (Samsung, Apple, Oppo...) liên tục ra mẫu mới, giảm giá để cạnh tranh giành khách hàng.",
    imageUrl: "/images/a.jpg",
    referenceUrl:
      "https://nguyentrungba.edu.vn/vi-du-ve-thi-truong-canh-tranh-hoan-hao",
  },
  {
    id: "b",
    title:
      'b. Hướng tới mục tiêu "dân giàu, nước mạnh, dân chủ, công bằng, văn minh":',
    icon: "fas fa-flag-checkered",
    subheading: "“La bàn” định hướng xã hội",
    explanation:
      'Đây chính là cái "la bàn" định hướng XHCN. Chúng ta dùng cái chợ (thị trường) không chỉ để làm giàu cho một nhóm người, mà là công cụ để cả xã hội cùng phát triển tốt đẹp hơn, công bằng hơn.',
    example:
      "Nhà nước vẫn đầu tư làm đường, kéo điện đến những vùng sâu vùng xa dù tốn kém, không có lãi ngay. Có chính sách BHYT cho người nghèo, người dân tộc thiểu số để đảm bảo ai cũng được chăm sóc sức khỏe cơ bản.",
    imageUrl: "/images/b.jpg",
    referenceUrl:
      "https://thuviennhadat.vn/phap-ly-nha-dat/bo-xay-dung-co-trach-nhiem-gi-trong-viec-quan-ly-tai-san-ket-cau-ha-tang-duong-sat-do-nha-nuoc-dau-t-696407.html",
  },
  {
    id: "c",
    title:
    ...
    explanation:
      'Để "cái chợ" hoạt động hiệu quả mà không gây ra hỗn loạn hay bất công quá lớn, Nhà nước ("trọng tài") sẽ can thiệp bằng luật pháp, chính sách. Và đường lối tổng thể, mục tiêu lớn do Đảng Cộng sản Việt Nam ("người chỉ đường") vạch ra.',
    example:
      "Nhà nước quy định mức lương tối thiểu để bảo vệ người lao động. Đảng đề ra Chiến lược phát triển kinh tế-xã hội 10 năm (ví dụ 2021-2030) làm định hướng cho các chính sách của Nhà nước.",
    imageUrl: "/images/c.jpg",
    referenceUrl:
      "https://thuvienphapluat.vn/lao-dong-tien-luong/chien-luoc-phat-trien-kinh-te--xa-hoi-10-nam-20212030-neu-phuong-huong-nhiem-vu-giai-phap-day-manh--34187.html",
  },
];
    ...
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      >
        <span className="flex items-center">
          <i
            className={`${item.icon} mr-3 text-xl w-6 text-center text-brand-navy`}
          ></i>
          {item.title}
        </span>
        <i
      ...
      >
        <div className="p-4 pl-12 pr-5 text-gray-700 space-y-4">
          {item.subheading && (
            <h3 className="font-bold text-lg text-brand-dark-gray italic">
              {item.subheading}
            </h3>
          )}
          ...
          {item.referenceUrl && (
            <div className="text-right text-sm pt-2">
              ...
              >
                Nguồn tham khảo{" "}
                <i className="fas fa-up-right-from-square ml-1.5 text-xs"></i>
              </a>
            </div>
          )}
        </div>
...
const Introduction: React.FC = () => {
  ...
  const [policyInput, setPolicyInput] = useState<string>(
    "Tăng thuế môi trường đối với các doanh nghiệp sản xuất thép."
  );
  const [simulationResult, setSimulationResult] = useState<{
    marketImpact: string;
    socialistImpact: string;
    stateImpact: string;
  } | null>(null);
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              marketImpact: {
                type: Type.STRING,
                description: "Tác động đến quy luật thị trường",
              },
              socialistImpact: {
                type: Type.STRING,
                description: "Tác động đến định hướng XHCN",
              },
              stateImpact: {
                type: Type.STRING,
                description: "Tác động đến vai trò của Nhà nước",
              },
            },
    ...
    <section className="mb-12">
      <Card>
        ...
        <div className="space-y-4 text-justify text-gray-700">
          <p>
            <strong>KTTT ĐHXHCN là gì?</strong> Như đã học ở Chương 2, KTTT là
            sản phẩm của văn minh nhân loại, là giai đoạn phát triển cao của
            kinh tế hàng hóa. Đặc trưng cơ bản của nó là sự vận hành theo các
            quy luật thị trường (cung-cầu, cạnh tranh, giá trị...).
          </p>
          <p>
            Tuy nhiên, không có mô hình KTTT chung cho mọi quốc gia. Mỗi nước sẽ
            có mô hình phù hợp với điều kiện lịch sử, chính trị, kinh tế - xã
            hội của mình.
          </p>
          <blockquote className="border-l-4 border-brand-navy/20 pl-4 py-3 my-6 bg-brand-navy/10 text-brand-dark-gray italic">
            <strong>Vậy, KTTT ĐHXHCN ở Việt Nam được định nghĩa là:</strong> Nền
          ...
          <p>
            "Định hướng XHCN" ở đây thực chất là hướng tới các giá trị cốt lõi
            đó. Đây là những giá trị mà nhân loại nói chung và Việt Nam nói
            riêng đang phấn đấu. Một điểm nhấn quan trọng là vai trò điều tiết
            của Nhà nước và sự lãnh đạo của Đảng Cộng sản Việt Nam. Đây là yếu
            tố đảm bảo cho thị trường vận hành đúng định hướng, phục vụ mục tiêu
            phát triển đất nước.
          </p>
          <p className="font-semibold text-gray-800 mt-6">
            Mô hình này vừa bao hàm đặc trưng chung của KTTT thế giới, vừa có
            đặc trưng riêng, phù hợp với lịch sử, trình độ phát triển của Việt
            Nam. Sau đây chúng ta tìm hiểu sâu hơn từng ý chính với ví dụ thực
            tế:
          </p>
        </div>
        ...
        <div className="mt-8 bg-gray-100 p-6 rounded-lg">
          ...
          <p className="text-gray-700">
            KTTT ĐHXHCN ở Việt Nam là sự kết hợp giữa việc sử dụng các công cụ
            hiệu quả của thị trường (cạnh tranh, cung - cầu) với vai trò quản
            lý, định hướng của Nhà nước dưới sự lãnh đạo của Đảng, nhằm đạt mục
            tiêu cuối cùng là xây dựng một xã hội phát triển, công bằng, văn
            minh.
          </p>
        </div>
        ...
        <div className="mt-12 border-t pt-8">
          ...
          <p className="text-center text-gray-600 mb-6 max-w-2xl mx-auto">
            Hãy thử đóng vai là nhà hoạch định chính sách. Đề xuất một chính
            sách và xem AI phân tích các tác động giả định của nó.
          </p>
              ...
              <div className="mt-8 space-y-4 animate-fadeIn">
                <div className="p-4 bg-gray-50 border-l-4 border-blue-500 rounded-r-lg">
                  <h4 className="font-bold text-gray-800 mb-1 flex items-center">
                    <i className="fas fa-chart-line w-6 text-center mr-2 text-blue-600"></i>
                    Tác động thị trường:
                  </h4>
                  <p className="text-gray-700">
                    {simulationResult.marketImpact}
                  </p>
                </div>
                <div className="p-4 bg-gray-50 border-l-4 border-green-500 rounded-r-lg">
                  <h4 className="font-bold text-gray-800 mb-1 flex items-center">
                    <i className="fas fa-flag-checkered w-6 text-center mr-2 text-green-600"></i>
                    Tác động XHCN:
                  </h4>
                  <p className="text-gray-700">
                    {simulationResult.socialistImpact}
                  </p>
                </div>
                <div className="p-4 bg-gray-50 border-l-4 border-purple-500 rounded-r-lg">
                  <h4 className="font-bold text-gray-800 mb-1 flex items-center">
                    <i className="fas fa-landmark w-6 text-center mr-2 text-purple-600"></i>
                    Tác động Nhà nước:
                  </h4>
                  <p className="text-gray-700">
                    {simulationResult.stateImpact}
                  </p>
                </div>
              </div>
            )}
